...
import type { MdNode, ImageRef, DocxContext, DocxFetchedImage, DocxImageAdapter } from './types.js'
...
export async function resolveAndFetchImages(
  ...
  options: {
    batchSize?: number
    adapter?: DocxImageAdapter
  } = {},
): Promise<{ urls: Map<string, ResolvedAttachment>; imageBuffers: Map<string, DocxFetchedImage> }> {
  ...
  for (const idChunk of chunk(ids, batchSize)) {
    ...
    if (!adapter) continue
    const res = await adapter.resolve(docId, idChunk)
  ...
  const imageBuffers = new Map<string, DocxFetchedImage>()
  ...
  for (const ref of imageRefs) {
    ...
    if (!url && ref.src) {
      ...
      if (/^data:/i.test(ref.src)) {
        ...
        const safe = adapter?.sanitizeDirectUrl?.(ref.src) ?? null
        if (safe) url = safe
      }
    }
    if (url) {
  ...
  for (let i = 0; i < fetchQueue.length; i += CONCURRENCY) {
    ...
    const results = await Promise.allSettled(
      batch.map(async ({ url, key }) => {
        ...
        try {
          if (!adapter) return
          const fetched = await adapter.fetch(url, MAX_BYTES)
          if (!fetched) return
          const image = fetched instanceof ArrayBuffer ? { data: fetched } : fetched
          if (image.data.byteLength <= MAX_BYTES) imageBuffers.set(key, image)
        } catch {
...
export function getImageBuffer(node: MdNode, ctx: DocxContext): ArrayBuffer | undefined {
  return getImageAsset(node, ctx)?.data
}

export function getImageAsset(node: MdNode, ctx: DocxContext): DocxFetchedImage | undefined {
  const attachId = typeof node.attrs?.attachId === 'string' ? node.attrs.attachId : undefined
  const src = typeof node.attrs?.src === 'string' ? node.attrs.src : undefined

  if (attachId) {
    const image = ctx.imageBuffers.get(attachId)
