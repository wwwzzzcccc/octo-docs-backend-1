...
import { FONT_CODE, FONT_EMOJI } from './styles.js'
...
export function normalizeDocxColor(raw: string): string | null {
  ...
  if (m) {
    ...
    if (parts.every((p) => p !== null)) return parts.join('').toUpperCase()
  }
 ...
 */
export function buildTextRuns(text: string, baseOpts: IRunOptions, _heading = false): TextRun[] {
  if (!text) return []
  // Script defaults live in the document/heading styles' independent
  // ascii/hAnsi/eastAsia slots. Do not stamp fallback faces onto each run: doing
  // so would turn an implicit fallback into an explicit font on re-import.
  const opts = baseOpts
  // Fast path: no emoji → single run.
  if (!EMOJI_RE.test(text)) return [new TextRun({ text, ...opts })]
  ...
  for (const part of parts) {
    ...
    runs.push(
      new TextRun(
        isEmoji
          ? { text: part.replace(/\uFE0F/gu, ''), ...opts, font: FONT_EMOJI }
          : { text: part, ...opts },
      ),
    )
  }
    ...
    switch (mark.type) {
      ...
      case 'textStyle': {
        ...
        if (typeof fontSize === 'string') {
          ...
          const unit = fontSize.trim().match(/[a-z%]+$/i)?.[0]?.toLowerCase() ?? 'px'
          const pt = unit === 'pt' ? value : unit === 'px' ? value * 0.75 : value
...
export function extractLinkHref(marks: MarkDef[]): string | null {
  ...
  const trimmed = href
    .replace(/^[\s\x00-\x20]+/, '')
    // Canonicalize an already XML-escaped relationship target before docx
    // performs the one required OOXML escaping pass.
    .replace(/&(?:amp;)+/gi, '&')
  ...
  for (const node of nodes) {
    switch (node.type) {
        ...
        if (linkHref) {
          runs.push(
            new ExternalHyperlink({
              children: buildTextRuns(text, { ...runOpts, style: 'Hyperlink' }, heading),
              link: linkHref,
            }),
          )
        } else {
          runs.push(...buildTextRuns(text, runOpts, heading))
        }
        break
      }
      case 'hardBreak':
      ...
      case 'emoji': {
        ...
        runs.push(...buildTextRuns(emojiText, {}, heading))
        break
      }
      case 'image': {
      ...
      default: {
        ...
        if (node.content && node.content.length) {
          runs.push(...convertInlineContent(node.content, emojiGlyph, heading))
        } else if (node.text) {
