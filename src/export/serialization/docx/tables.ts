...
import { convertInlineContent } from './marks.js'
import { convertImage } from './nodes.js'
...
export function convertTable(node: MdNode, ctx: DocxContext, availWidthDxa?: number): Table {
  ...
  const tableRows = rows.map((row, rowIdx) => {
    ...
    const h = Number(row.attrs?.height)
    return new TableRow({ children: cells, tableHeader: isHeaderRow || undefined, height: Number.isFinite(h) && h > 0 ? { value: Math.round(h * 15), rule: 'atLeast' } : undefined })
  })
