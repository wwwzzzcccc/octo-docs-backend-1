...
function sanitizeLineHeight(v: unknown): string | null { if (typeof v !== 'string' && typeof v !== 'number') return null; const s=String(v).trim(); const n=Number(s); return Number.isFinite(n) && n > 0 && n <= 10 ? s : null }
function sanitizeSpacing(v: unknown): string | null { if (typeof v !== 'string') return null; const s=v.trim(); const m=/^(\d+(?:\.\d+)?)(px|em)$/.exec(s); return m && Number(m[1]) <= 1000 ? s : null }

/** Default font for body text. */
export const FONT_BODY = 'SimSun'

/** Default font for headings. */
export const FONT_HEADING = 'Microsoft YaHei'
...
export function createDefaultStyles(): NonNullable<IStylesOptions['default']> {
  return {
    document: {
      run: {
        font: { ascii: 'Times New Roman', hAnsi: 'Times New Roman', eastAsia: FONT_BODY, cs: 'Times New Roman' },
    ...
    heading1: {
      run: {
        font: { ascii: 'Arial', hAnsi: 'Arial', eastAsia: FONT_HEADING, cs: 'Arial' },
