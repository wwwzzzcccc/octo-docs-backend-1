 ...
 */
function parseHtmlTable(html: string, inlineMap: InlineMapper, warnings: string[]): PmNode | null {
...
function domTableToNode(tableEl: DomElement, inlineMap: InlineMapper, warnings: string[], depth = 0): PmNode {
  ...
  for (const tr of trEls) {
    ...
    for (const cellEl of Array.from(tr.children)) {
      ...
      const blocks = domCellToBlocks(cellEl, inlineMap, warnings, depth)
      const cell: PmNode = {
...
function domCellToBlocks(
  ...
  for (const child of Array.from(cellEl.childNodes)) {
    ...
    if (tag === 'table') {
      flushInline()
      out.push(domTableToNode(el, inlineMap, warnings, depth + 1))
    } else if (tag === 'img') {
      flushInline()
      ...
      out.push({
        ...
        content: inlineMap((el.innerHTML || '').trim(), warnings),
      })
    } else if (tag === 'pre') {
      flushInline()
      ...
      flushInline()
      const inner = domCellToBlocks(el, inlineMap, warnings, depth + 1)
      out.push({
        type: 'blockquote',
