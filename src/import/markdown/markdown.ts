...
import { isSafeHref, parseInlineHtml, parseHtmlBlock, type BlockMapper } from './html-inline.js'
...
function md(): MarkdownIt {
  ...
  mdInstance = new MarkdownIt('commonmark', {
    html: true,
    linkify: false,
    breaks: false,
...
export function parseMarkdownToPmDoc(input: string, opts: ImportOptions = {}): ImportResult {
  ...
  // HTML fallback tables can contain explicit empty <p></p> blocks. Their
  // recursive inline mapper previously represented those as illegal empty text
  // nodes, which made the authoritative apply path fail with
  // `schema_incompatible`. Preserve the empty paragraph but remove only empty
  // text leaves before ProseMirror validates the result.
  pruneEmptyTextNodes(content)
  ...
  if (ctx.localImageCount > 0) {
    warnings.push(
      tr(opts, 'docs.import.localImagesSkipped', {
        count: ctx.localImageCount,
      }),
    )
  }
 ...
 */
function tr(opts: ImportOptions, key: string, params?: Record<string, string | number>): string {
 ...
 */
function mapBlockTokens(tokens: Token[], ctx: Ctx, depth = 0): PmNode[] {
  if (depth > MAX_BLOCK_NESTING) {
    ctx.warnings.push('docs.import.markdownNestingLimit')
  ...
  while (i < tokens.length) {
    ...
    switch (tok.type) {
      ...
      case 'paragraph_open': {
        ...
        const rawText = rawInline && rawInline.type === 'inline' ? rawInline.content : ''
      ...
      case 'bullet_list_open': {
        const { node, next } = takeList(tokens, i, 'bulletList', ctx, depth)
        out.push(node)
        i = next
        break
      }
      case 'ordered_list_open': {
        const { node, next } = takeList(tokens, i, 'orderedList', ctx, depth)
        out.push(node)
        i = next
        break
      }
      case 'blockquote_open': {
        ...
        const inner = mapBlockTokens(tokens.slice(i + 1, end), ctx, depth + 1)
        out.push({
          type: 'blockquote',
      ...
      case 'html_block': {
        ...
        const blockMap: BlockMapper = (markdown) =>
          mapBlockTokens(md().parse(markdown, {}), {
            ...ctx,
            sourceLines: markdown.split('\n'),
          }, depth + 1)
        const assembled = tryAssembleHtmlBlock(tokens, i, ctx)
        if (assembled) {
          const nodes = parseHtmlBlock(
            assembled.html,
            (t) => mapInlineHtmlText(t, ctx),
            ctx.warnings,
            blockMap,
          )
...
function tryAssembleHtmlBlock(
  ...
  for (let j = openIdx + 1; j < tokens.length; j++) {
    const t = tokens[j]
    if (!t || t.type !== 'html_block') continue
...
function takeList(
  ...
  while (j < end) {
    if (tokens[j]?.type === 'list_item_open') {
      ...
      const itemBlocks = mapBlockTokens(tokens.slice(j + 1, itemEnd), ctx, depth + 1)
  ...
  if (kind === 'orderedList') {
    ...
    return {
      node: attrs ? { type: kind, attrs, content: items } : { type: kind, content: items },
      next: end + 1,
    }
  }
...
function orderedListStart(openTok: Token | undefined): number | null {
 ...
 *   unchecked: [ ]
 */
function extractTask(blocks: PmNode[]): {
  isTask: boolean
  checked: boolean
  content: PmNode[]
} {
...
function mapTable(inner: Token[], ctx: Ctx): PmNode {
  ...
  while (i < inner.length) {
    const t = inner[i]
    if (!t) break
    if (t.type === 'thead_open' || t.type === 'thead_close') {
      i++
      continue
    }
    ...
    if (t.type === 'tr_open') {
      ...
      let j = i + 1
      while (j < trEnd) {
        const cellTok = inner[j]
        if (!cellTok) {
          j++
          continue
        }
        if (cellTok.type === 'th_open' || cellTok.type === 'td_open') {
          const isHeader = cellTok.type === 'th_open'
          const closeType = isHeader ? 'th_close' : 'td_close'
          const cellEnd = matchClose(inner, j, cellTok.type, closeType)
...
function mapInline(inlineTok: Token, ctx: Ctx): PmNode[] {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    switch (c.type) {
      ...
      case 'link_close':
        popMark(markStack, 'link', '__dropped_link')
        break
      case 'image': {
...
function mapImage(tok: Token, ctx: Ctx): PmNode | null {
  ...
  if (!isNetwork) {
    ...
    return {
      type: 'text',
      text: tr(ctx.opts, 'docs.import.imageNotImported', {
        name: alt || src || '',
      }),
    }
  }
...
function pushText(out: PmNode[], text: string, marks: PmNode['marks'], ctx: Ctx): void {
  if (!text) return
  let last = 0
  let m: RegExpExecArray | null
...
function pruneEmptyTextNodes(nodes: PmNode[]): void {
  for (const node of nodes) {
    if (!node.content) continue
    node.content = node.content.filter((child) => child.type !== 'text' || Boolean(child.text))
...
function mapInlineHtmlText(text: string, _ctx: Ctx): PmNode[] {
