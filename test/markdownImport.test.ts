...
import { parseMarkdownToPmDoc, type PmNode } from '../src/import/markdown/markdown.js'
import { isSafeHref, isSafeCssColor } from '../src/import/markdown/html-inline.js'
import { stripFrontMatter } from '../src/import/markdown/frontmatter.js'
...
describe('parseMarkdownToPmDoc — block nodes', () => {
  it('parses headings with correct level', () => {
    ...
    expect(r.doc.content![0]).toMatchObject({
      type: 'heading',
      attrs: { level: 1 },
    })
    expect(r.doc.content![1]).toMatchObject({
      type: 'heading',
      attrs: { level: 2 },
    })
    expect(r.doc.content![2]).toMatchObject({
      type: 'heading',
      attrs: { level: 3 },
    })
  })
  ...
  it('parses task lists', () => {
    ...
    expect(firstBlock(r).content![0]).toMatchObject({
      type: 'taskItem',
      attrs: { checked: false },
    })
    expect(firstBlock(r).content![1]).toMatchObject({
      type: 'taskItem',
      attrs: { checked: true },
    })
  })
  ...
  it('maps a checkmark glyph in the checkbox to a checked task item', () => {
    ...
    expect(firstBlock(r).content![2]).toMatchObject({
      type: 'taskItem',
      attrs: { checked: false },
    })
  })
...
describe('parseMarkdownToPmDoc — inline marks', () => {
  ...
  it('parses <mark style="background-color:..."> into a colored highlight', () => {
    ...
    const hl = p.content!.find((n) => n.marks?.some((m) => m.type === 'highlight'))
    expect(hl).toBeDefined()
    expect(hl!.marks).toContainEqual({
      type: 'highlight',
      attrs: { color: '#fff3a3' },
    })
  })
  ...
  it('parses emoji shortcodes when the resolver confirms them', () => {
    const r = parseMarkdownToPmDoc('Hello :smile: world', {
      emojiName: (n) => (n === 'smile' ? '😄' : undefined),
    })
    const p = firstBlock(r)
  ...
  it('keeps unknown emoji shortcodes as literal text', () => {
    const r = parseMarkdownToPmDoc('a :not_a_real_emoji: b', {
      emojiName: () => undefined,
    })
    const p = firstBlock(r)
...
describe('parseMarkdownToPmDoc — images', () => {
  ...
  it('keeps text and a block image as separate blocks in the same cell', () => {
    const md = ['| c |', '| --- |', '| hi ![p](https://example.com/y.png) |'].join('\n')
    const r = parse(md)
    const table = r.doc.content!.find((n) => n.type === 'table')!
    const bodyRow = table.content!.find((row) => row.content!.some((c) => c.type === 'tableCell'))!
  ...
  it('does not emit an empty text node for an empty <pre> in a table cell (white-screen guard)', () => {
    ...
    const table = r.doc.content!.find((n) => n.type === 'table')!
    const bodyRow = table.content!.find((row) => row.content!.some((c) => c.type === 'tableCell'))!
    const cb = bodyRow.content!.flatMap((c) => c.content ?? []).find((b) => b.type === 'codeBlock')!
