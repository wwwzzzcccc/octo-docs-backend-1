...
describe('markdown import — dollar math', () => {
  ...
  it('lifts an inline $$…$$ out of the paragraph so it is a schema-valid block node', () => {
    ...
    const res = parseMarkdownToPmDoc('foo $$E=mc^2$$ bar') as {
      doc?: { content?: unknown[] }
    }
    const doc = (res.doc ?? res) as {
  ...
  it('lifts a $$…$$ block out of a list item paragraph', () => {
    const res = parseMarkdownToPmDoc('- item with $$x^2$$ here') as {
      doc?: unknown
    }
    const doc = res.doc ?? res
    ...
    const walk = (n: unknown): void => {
      ...
      if (node.type === 'paragraph' && Array.isArray(node.content)) {
        for (const c of node.content)
          if ((c as { type?: string }).type === 'blockMath') nestedInParagraph++
      }
      if (Array.isArray(node.content)) node.content.forEach(walk)
    }
    walk(doc)
